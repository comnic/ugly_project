<?php
    ...
    class Naver_oauth{
        ...
        function __construct() {
            ...
            $this -> client_id = $this->ci->config->item('consumer_key', 'naver');
            $this -> client_secret = $this->ci->config->item('consumer_key_secret', 'naver');;
            $this -> redirect_url = $this->ci->config->item('redirect_url', 'naver');;
            if(!isset($_SESSION)) {
                session_start();
            }
        }
        ...
        public function set_state(){
            $this -> generate_state();
            $this->ci->session->set_userdata('state', $this -> state);
        }
        private function get_code(){
            $this -> code = $_GET['code'];
        }
        private function get_state(){
            $this -> state = $this->ci->session->userdata('state');
            return $this -> state;
        }
...
//            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false );
            ...
            $this -> userInfo = array(
                'userID' => explode("@", (string)$xml -> response -> email ),
